...
if uploaded_file:
    ...
    def draw_meal(idx,name,data):
        ...
        for i,(ing,qty) in enumerate(data["ingredients"].items()):
            total_qty=qty*tot; bt=round(total_qty/batches) if batches else 0; bl=str(batches) if i==0 else ""
        ...
        if "sub_section" in data:
            ...
            for ingr,per in sub["ingredients"].items(): total_qty=per*tot; adj=round(total_qty/batches) if batches else round(total_qty,2); pdf.set_x(x); pdf.cell(col_w*0.3,ch,ingr[:20],1); pdf.cell(col_w*0.15,ch,str(per),1); pdf.cell(col_w*0.15,ch,str(tot),1); pdf.cell(col_w*0.25,ch,str(adj),1); pdf.cell(col_w*0.15,ch,"",1); pdf.ln(ch)
        heights[idx]=pdf.get_y()+pad
    ...
    for idx,(name,data) in enumerate({
        ...
        x=xpos[idx]; y=pdf.get_y(); pdf.set_xy(x,y)
        pdf.set_font("Arial","B",11); pdf.set_fill_color(230,230,230); pdf.cell(col_w,ch,name,ln=1,fill=True)
        pdf.set_x(x); pdf.set_font("Arial","B",8)
    ...
    pdf.set_font("Arial","",8)
    for sauce,qty,meal_key in [("MONGOLIAN",70,"MONGOLIAN BEEF"),("MEATBALLS",120,"BEEF MEATBALLS"),("LEMON",50,"ROASTED LEMON CHICKEN"),("MUSHROOM",100,"STEAK WITH MUSHROOM SAUCE"),("FAJITA SAUCE",33,"CHICKEN FAJITA BOWL"),("BURRITO SAUCE",43,"BEEF BURRITO BOWL")]: pdf.set_x(left); amt=meal_totals.get(meal_key.upper(),0); total=qty*amt; pdf.cell(col_w*0.4,ch,sauce,1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt),1); pdf.cell(col_w*0.2,ch,str(total),1); pdf.ln(ch)

    # ------------------
    # Chicken Mixing + All Mixes
    # ------------------
    pdf.ln(5); pdf.set_font("Arial","B",14); pdf.cell(0,10,"Chicken Mixing",ln=1,align='C'); pdf.ln(5)
    mixes=[("Pesto",[("Chicken",110),("Sauce",80)],"CHICKEN PESTO PASTA"),("Butter Chicken",[("Chicken",120),("Sauce",90)],"BUTTER CHICKEN"),("Broccoli Pasta",[("Chicken",100),("Sauce",100)],"CHICKEN AND BROCCOLI PASTA"),("Thai",[("Chicken",110),("Sauce",90)],"THAI GREEN CHICKEN CURRY")]
    for mix_title,ingredients,meal_key in mixes:
        pdf.set_x(left); pdf.set_font("Arial","B",11); pdf.set_fill_color(230,230,230); pdf.cell(col_w,ch,mix_title,ln=1,fill=True); pdf.ln(2)
        ...
        pdf.ln(ch); pdf.set_font("Arial","",8)
        amt=meal_totals.get(meal_key.upper(),0); batches=math.ceil(amt/50) if amt>0 else 0
