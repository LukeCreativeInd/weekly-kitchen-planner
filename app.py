...
if uploaded_file:
    ...
    def draw_meal(idx,name,data):
        ...
        for i,(ing,qty) in enumerate(data.get("ingredients",{}).items()):
            ...
            pdf.set_x(x); pdf.cell(col_w*0.3,ch,ing[:20],1); pdf.cell(col_w*0.15,ch,str(qty),1); pdf.cell(col_w*0.15,ch,str(tot),1); pdf.cell(col_w*0.25,ch,str(bt),1); \
pdf.cell(col_w*0.15,ch,bl,1); pdf.ln(ch)
        if "sub_section" in data:
            ...
            for ingr,per in sub.get("ingredients",{}).items(): total_qty=per*tot; adj=round(total_qty/batches) if batches else round(total_qty,2); pdf.cell(col_w*0.3,ch,ingr[:20],1); pdf.cell(col_w*0.15,ch,str(per),1); pdf.cell(col_w*0.15,ch,str(tot),1); pdf.cell(col_w*0.25,ch,str(adj),1); pdf.cell(col_w*0.15,ch,"",1); pdf.ln(ch)
        heights[idx]=pdf.get_y()+pad
    for name,data in meal_recipes.items():
        main=len(data.get("ingredients",{})); sub=len(data.get("sub_section",{}).get("ingredients",{}))
