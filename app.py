...
def draw_recipe():
    ...
    pdf.add_page()
    pdf.set_font("Arial","B",14)
    pdf.cell(0,10,title2,ln=1,align='C')
    pdf.ln(5)
    hts = [pdf.get_y(), pdf.get_y()]
    c = 0
    for name, data in meal_recipes.items():
        ...
        blk = rows * ch + pad
        hts, c = next_pos(hts, c, blk, title2)
        x, y = xpos[c], hts[c]
        pdf.set_xy(x,y)
        pdf.set_font("Arial","B",11)
        pdf.set_fill_color(230,230,230)
...
for header,w in [("Sauce",0.4),("Qty",0.2),("Amt",0.2),("Total",0.2)]: pdf.cell(col_w*w,ch,header,1)
pdf.ln(ch); pdf.set_font("Arial","",8)
for sauce,qty,meal_key in sauce_prep:
    pdf.set_x(xpos[0]); amt = meal_totals.get(meal_key.upper(),0);
    pdf.cell(col_w*0.4,ch,sauce,1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt),1); pdf.cell(col_w*0.2,ch,str(qty*amt),1);
    pdf.ln(ch)
# Table2: Beef Burrito Mix
...
for header,w in [("Ingredient",0.3),("Qty",0.2),("Amt",0.2),("Total",0.2),("Batch",0.1)]: pdf.cell(col_w*w,ch,header,1)
pdf.ln(ch); pdf.set_font("Arial","",8)
...
for ingr,qty in bb_rows:
    pdf.set_x(xpos[1]); total=(qty*amt_bb)/batches_bb if batches_bb else 0;
    pdf.cell(col_w*0.3,ch,ingr,1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt_bb),1); pdf.cell(col_w*0.2,ch,str(round(total,2)),1); pdf.cell(col_w*0.1,ch,str(batches_bb),1)
    pdf.ln(ch)
# Table3: Parma Mix
pm_rows=[("Napoli Sauce",50),("Mozzarella Cheese",40)]
y2 = max(pdf.get_y(), start_y+len(sauce_prep)*ch+pad)
pdf.set_xy(xpos[0], y2)
pdf.set_font("Arial","B",11); pdf.set_fill_color(230,230,230)
pdf.cell(col_w,ch,"Parma Mix",ln=1,fill=True)
pdf.set_x(xpos[0]); pdf.set_font("Arial","B",8)
for header,w in [("Ingredient",0.4),("Qty",0.3),("Amt",0.3)]: pdf.cell(col_w*w,ch,header,1)
pdf.ln(ch); pdf.set_font("Arial","",8)
amt_pm=meal_totals.get("NAKED CHICKEN PARMA",0)
for ingr,qty in pm_rows:
    pdf.set_x(xpos[0]); pdf.cell(col_w*0.4,ch,ingr,1); pdf.cell(col_w*0.3,ch,str(qty),1); pdf.cell(col_w*0.3,ch,str(amt_pm),1); pdf.ln(ch)

# Mixing start below fridge
mix_start = max(y2+(len(pm_rows)+2)*ch+pad, pdf.get_y())
pdf.set_xy(left, mix_start)
pdf.set_font("Arial","B",14)
pdf.cell(0,10,"Chicken Mixing",ln=1,align='C')
pdf.ln(5)
# Chicken Mixing Tables (2-col)
mixes=[
