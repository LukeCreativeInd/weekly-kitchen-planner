...
# Upload Section
st.title("Weekly Kitchen Planner")
...
if uploaded_file:
    ...
    # SUM MEALS
    meal_totals = df.groupby("Product name")[["Quantity"]].sum().to_dict()["Quantity"]
    ...
    pdf.cell(0, 10, "Daily Production Report", ln=True, align="C")
    pdf.ln(5)

    # -- SAUCE SECTION ON PAGE 3 --
    pdf.add_page()
    pdf.set_font("Arial", "B", 12)
    pdf.cell(0, 10, "Sauce Requirements", ln=True, align="C")
    pdf.ln(3)
    ...
    pdf_path = f"/mnt/data/daily_production_report_{filename_date}.pdf"
    pdf.output(pdf_path)

    with open(pdf_path, "rb") as f:
        st.download_button("\U0001F4C4 Download Final PDF", f, file_name=os.path.basename(pdf_path), mime="application/pdf")
