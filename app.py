...
# ---- Page 1: Bulk Summary ----
t1 = f"Daily Production Report - {datetime.today().strftime('%d/%m/%Y')}"
pdf.add_page()
pdf.set_font("Arial","B",14)
pdf.cell(0,10,t1,ln=1,align='C')
pdf.ln(5)
heights = [pdf.get_y(), pdf.get_y()]
col = 0
for sec in bulk_sections:
    block_h = (len(sec['ingredients']) + 2) * ch + pad
    heights, col = next_pos(heights, col, block_h, t1)
...
pdf.add_page()
pdf.set_font("Arial","B",14)
pdf.cell(0,10,t2,ln=1,align='C')
pdf.ln(5)
heights = [pdf.get_y(), pdf.get_y()]
col = 0
for name, data in meal_recipes.items():
    rows = 2 + len(data['ingredients']) + (2 + len(data.get('sub_section', {}).get('ingredients', {})) if 'sub_section' in data else 0)
    block_h = rows * ch + pad
    heights, col = next_pos(heights, col, block_h, t2)
    x, y = xpos[col], heights[col]
    pdf.set_xy(x, y)
    pdf.set_font("Arial","B",11)
    pdf.set_fill_color(230,230,230)
...
for idx, (name, data) in enumerate(sauces.items()):
    ...
    pdf.ln(ch)
    pdf.set_font("Arial","",8)
    total_meals = meal_totals.get(data['meal_key'], 0)
    for ing, amt in data['ingredients']:
        pdf.set_x(x)
        pdf.cell(col_w*0.3, ch, ing[:20], 1)
        pdf.cell(col_w*0.2, ch, str(amt), 1)
        pdf.cell(col_w*0.2, ch, str(total_meals), 1)
        pdf.cell(col_w*0.3, ch, str(amt * total_meals), 1)
        pdf.ln(ch)
    heights.append(pdf.get_y())
# Position For To Pack In Fridge Title
fridge_start = max(heights) + pad
...
tables = [
    ("Sauces to Prepare", [("MONGOLIAN",70,"MONGOLIAN BEEF"),("MEATBALLS",120,"BEEF MEATBALLS"),("LEMON",50,"ROASTED LEMON CHICKEN"),("MUSHROOM",100,"STEAK WITH MUSHROOM SAUCE"),("FAJITA SAUCE",33,"CHICKEN FAJITA BOWL")], ["Sauce","Qty","Amt","Total"]),
    ("Beef Burrito Mix", [("Salsa",43),("Black Beans",50),("Corn",50),("Rice",130)], ["Ingredient","Qty","Amt","Total","Batch"]),
    ("Parma Mix", [("Napoli Sauce",50),("Mozzarella Cheese",40)], ["Ingredient","Qty","Amt"]),
]
col_heights = [pdf.get_y(), pdf.get_y()]
col = 0
for title, rows, headers in tables:
    block_h = (len(rows) + 2) * ch + pad
    col_heights, col = next_pos(col_heights, col, block_h)
    x = xpos[col]
    ...
    # Draw headers
    widths = [0.4,0.2,0.2,0.2] if title=="Sauces to Prepare" else [0.3,0.2,0.2,0.2,0.1] if title=="Beef Burrito Mix" else [0.4,0.3,0.3]
    for hdr,w in zip(headers, widths): pdf.cell(col_w*w, ch, hdr, 1)
    pdf.ln(ch); pdf.set_font("Arial","",8)
    if title=="Sauces to Prepare":
        for s,qty,mk in rows:
            amt = meal_totals.get(mk.upper(),0)
            pdf.set_x(x); pdf.cell(col_w*0.4,ch,s,1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt),1); pdf.cell(col_w*0.2,ch,str(qty*amt),1); pdf.ln(ch)
        ...
        for ingr,qty in rows:
            total_bb = (qty*amt_bb)/batches_bb if batches_bb else 0
            pdf.set_x(x); pdf.cell(col_w*0.3,ch,ingr,1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt_bb),1); pdf.cell(col_w*0.2,ch,str(round(total_bb,2)),1); pdf.cell(col_w*0.1,ch,str(batches_bb),1); pdf.ln(ch)
    else:
        amt_pm = meal_totals.get("NAKED CHICKEN PARMA",0)
        for ingr,qty in rows:
            pdf.set_x(x); pdf.cell(col_w*0.4,ch,ingr,1); pdf.cell(col_w*0.3,ch,str(qty),1); pdf.cell(col_w*0.3,ch,str(amt_pm),1); pdf.ln(ch)
...
mix_heights = [pdf.get_y(), pdf.get_y()]
col = 0
for title, rows, mk, div in mixes:
    block_h = (len(rows) + 2) * ch + pad
    mix_heights, col = next_pos(mix_heights, col, block_h)
    x, y = xpos[col], mix_heights[col]
    pdf.set_xy(x, y)
    pdf.set_font("Arial","B",11)
    pdf.set_fill_color(230,230,230)
    pdf.cell(col_w, ch, title, ln=1, fill=True)
    pdf.set_x(x); pdf.set_font("Arial","B",8)
    for h, w in [("Ingredient",0.3),("Quantity",0.2),("Amount",0.2),("Total",0.2),("Batch",0.1)]: pdf.cell(col_w*w,ch,h,1)
    pdf.ln(ch); pdf.set_font("Arial","",8)
    amt = meal_totals.get(mk.upper(),0)
    rb = math.ceil(amt/div) if amt>0 else 0; batches = rb + (rb%2)
    for ingr,qty in rows:
        total = (qty*amt)/batches if batches else 0
        pdf.set_x(x); pdf.cell(col_w*0.3,ch,ingr[:20],1); pdf.cell(col_w*0.2,ch,str(qty),1); pdf.cell(col_w*0.2,ch,str(amt),1); pdf.cell(col_w*0.2,ch,str(round(total,2)),1); pdf.cell(col_w*0.1,ch,str(batches),1); pdf.ln(ch)
    mix_heights[col] = pdf.get_y() + pad
